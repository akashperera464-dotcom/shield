/**
 * SuperAdmin — CMS + Team Manager.
 *
 * Works in two modes:
 *   • Live mode (Firebase configured)  → reads/writes `siteContent/globalConfig`
 *                                        and `users` collection.
 *   • Demo mode (AuthContext.isDemo)   → renders mock data so the whole
 *     console can be reviewed before Firebase is connected.
 *
 * Step 6 will replace the mock data with real Firestore reads/writes.
 */
...
const DEMO_CONFIG = {
  ...
  aboutText: 'DevForge is a senior-only software studio. We design, build, and ship production React apps with Firebase, Cloudinary, and role-based admin tooling.',
  contactEmail: 'hello@devforge.agency',
  logoUrl: 'https://res.cloudinary.com/dhd06wdov/image/upload/v1784282735/ChatGPT_Image_Jul_17_2026_05_03_17_PM_adkeeh.png',
  mainBgUrl: 'https://res.cloudinary.com/dhd06wdov/image/upload/v1784282735/ChatGPT_Image_Jul_17_2026_05_03_17_PM_adkeeh.png',
}

const DEMO_TEAM = [
  { uid: 'u_001', name: 'You (Superadmin)', email: 'superadmin@demo.devforge',    role: 'superadmin', createdAt: '2026-06-01' },
  { uid: 'u_002', name: 'Daniel Okafor',    email: 'daniel@devforge.agency',      role: 'admin',      createdAt: '2026-06-12' },
  { uid: 'u_003', name: 'Mei Tanaka',       email: 'mei@devforge.agency',         role: 'admin',      createdAt: '2026-06-20' },
  { uid: 'u_004', name: 'Sara Al-Mansoori', email: 'sara@devforge.agency',        role: 'admin',      createdAt: '2026-07-01' },
]
...
function CMSPanel({ isDemo }) {
  const [form, setForm] = useState(DEMO_CONFIG)
  const [saved, setSaved] = useState(false)

  const handleSave = (e) => {
    e.preventDefault()
    // In demo mode we just flash a success state.
    setSaved(true)
...
function TeamPanel({ isDemo }) {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      if (isDemo) {
        // Just add to the local list for demo purposes.
        const newMember = {
    ...
    <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="mt-6 space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input value={name} onChange={(e) => setName(e.target.value)} required className="input-field pl-10" placeholder="Jane Doe" />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="input-field pl-10" placeholder="jane@devforge.agency" />
            </div>
          </div>
          <div>
            ...
            <div className="relative">
              ...
              <input type={show ? 'text' : 'password'} value={pass} onChange={(e) => setPass(e.target.value)} required minLength={8} className="input-field pl-10 pr-10" placeholder="Min. 8 characters" />
              <button type="button" onClick={() => setShow((s) => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-500 hover:text-ink-200" tabIndex={-1}>
      ...
      <div className="glass-card p-6">
        ...
        <div className="mt-4 space-y-3">
          {team.map((m) => (
            <div key={m.uid} className="flex items-center gap-3 rounded-xl border border-white/5 bg-white/[0.02] p-3">
              <div className={`flex h-9 w-9 items-center justify-center rounded-full font-display text-sm font-bold text-white ring-1 ring-white/10 ${
                m.role === 'superadmin'
